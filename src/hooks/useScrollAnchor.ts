...
export default function useScrollAnchor() {
  ...
  const scrollToBottom = React.useCallback(() => {
    if (messagesRef.current) messagesRef.current.scrollIntoView({ block: "end", behavior: "smooth" });
  }, []);
  ...
  return { messagesRef, scrollRef, visibilityRef, scrollToBottom, isAtBottom, isVisible };
}
